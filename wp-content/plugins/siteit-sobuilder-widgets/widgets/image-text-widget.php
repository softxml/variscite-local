<?php
class sgImgTextWidget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array(
			...
			'description'	=> __('A complete solution for mixing image with text block By SITEIT', 'siteitsob')
		);

		parent::__construct('sgImgTextWidget', __('Image & Text Widget (SiteIT)', 'siteitsob'), $widget_ops);
	}
	...
	function sgDataArr($type) {
		...
		elseif($type == 'valigns') {
			$type = array('flex flex-start' => __('Top', 'siteitsob'), 'flex flex-center' => __('Middle', 'siteitsob'), 'flex flex-end' => __('Bottom', 'siteitsob'));
		}
			...
			<div class="col-md-4">
				...
				<div class="row-wrap">
					<div class="admin-row">
						...
						<div class="col-md-4">
							<label for="'.$this->get_field_id($prefix.'image_border').'"><span class="label-wrap">'.__('Image Border', 'siteitsob').':</span>
						...
						<div class="col-md-4">
							<label for="'.$this->get_field_id($prefix.'image_bradius').'"><span class="label-wrap">'.__('Border Radius', 'siteitsob').':</span>
						...
						<div class="col-md-4">
							<label for="'.$this->get_field_id($prefix.'image_bcolor').'"><span class="label-wrap">'.__('Border Color', 'siteitsob').':</span>
						...
						<div class="col-md-12">
							<ul class="lsnone m0 p0">
								<li><label for="'.$this->get_field_id($prefix.'use_lightbox').'"><input class="widefat" id="'.$this->get_field_id($prefix.'use_lightbox').'" type="checkbox" name="'.$this->get_field_name($prefix.'use_lightbox').'" '.($instance[$prefix.'use_lightbox'] == 'on' ? 'checked' : '').' /> '.__('Enable Lightbox (overides "Image Link" property)', 'siteitsob').'</label></li>
							</ul>
						</div>		
			...
			<div class="col-md-4">
				...
				<div class="row-wrap">
					<div class="admin-row">
						...
						<div class="col-md-4">
							<label for="'.$this->get_field_id($prefix.'title_minheight').'"><span class="label-wrap">'.__('Min Height (px / %)', 'siteitsob').':</span>
								<input class="widefat" id="'.$this->get_field_id($prefix.'title_minheight').'" name="'.$this->get_field_name($prefix.'title_minheight').'" type="text" value="'.esc_attr($instance[$prefix.'title_minheight']).'" />
							</label>	
						</div>
						<div class="col-md-4">
						...
						<div class="col-md-4">
							<label for="'.$this->get_field_id($prefix.'title_link').'"><span class="label-wrap">'.__('Title Link', 'siteitsob').':</span>
						...
						<div class="col-md-4">
							<label for="'.$this->get_field_id($prefix.'title_ltarget').'"><span class="label-wrap">'.__('Link Target', 'siteitsob').':</span>
								<select class="widefat" id="'.$this->get_field_id($prefix.'title_ltarget').'" name="'.$this->get_field_name($prefix.'title_ltarget').'">
									'.$this->multi_select($instance[$prefix.'title_ltarget'], __('Default'), 'linkTargets').'
								</select>
							</label>
						</div>	
	...
	function widget($args, $instance) {
		...
		if(isset($instance[$prefix.'image_url'])) {
			...
			if($instance[$prefix.'use_lightbox'] == 'on') {$astart = '<a href="'.$instance[$prefix.'image_url'].'" title="'.$instance[$prefix.'widget_title'].'" class="lightbox">'; $aend = '</a>';} 
			...
			$image = '<div class="img-box '.$instance[$prefix.'image_valign'].' '.$instance[$prefix.'image_align'].'">'.$astart.'<img src="'.$instance[$prefix.'image_url'].'" alt="'.$instance[$prefix.'image_alt'].'" class="'.$imgClasses.'" style="'.$imgStyles.'" /> '.$aend.'</div>';				
		}
		...
		if(isset($instance[$prefix.'widget_title'])) {
			...
			if($instance[$prefix.'title_minheight']) {$titleCss .= 'min-height:'.$instance[$prefix.'title_minheight'].';'; }

			
			// title link
			if($instance[$prefix.'title_link']) {$ttlLinkStr = '<a href="'.$instance[$prefix.'title_link'].'" target="'.$instance[$prefix.'title_ltarget'].'">'; $ttlLinkEnd = '</a>';} else {$ttlLinkStr = ''; $ttlLinkEnd = '';}


			$widget_title = '
			<div class="title-box '.$instance[$prefix.'title_align'].' ">
				<'.$instance[$prefix.'title_type'].' class="widgettitle '.$instance[$prefix.'title_fweight'].' '.$instance[$prefix.'title_classes'].' " style="'.$titleCss.'; font-size: '.$instance[$prefix.'title_size'].'px; display: inline-block;">'.$ttlLinkStr.$instance[$prefix.'widget_title'].$ttlLinkEnd.'</'.$instance[$prefix.'title_type'].'>
			</div>
			';
		}
		...
		$result = '
		<div class="singleWidget sitBuilderWidget sitBuilderImageTextWidget '.$instance[$prefix.'widget_classes'].'" style="'.$widgetCss.'">
			'.$image.'
			'.$widget_title.'
			'.$text.'
		</div>
...
// add_action( 'widgets_init', create_function('', 'return register_widget("sgImgTextWidget");') );
add_action( 'widgets_init', function () {
    return register_widget('sgImgTextWidget');
}, 1 );
?>
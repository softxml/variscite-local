    ...
    if($actionType == 'send_quote') {
        ...
        foreach( $data as $key => $item ){
            if( $key == 'note' ){
                ...
                if(! wp_doing_ajax() && $key === 'quote-product') {
                    $email[$key] = sanitize_text_field(implode(';', $item));
                } else {
...
//		$email['amount'] 	= sanitize_text_field( implode(', ', $data['amount']) );
        ...
        if( strlen($email['first_name']) < 2 && wp_doing_ajax() ) { wp_send_json_error( array('msg' => __('Please enter a valid first name.') ) ); }
        elseif( !is_valid_email($email['email']) && wp_doing_ajax() ) { wp_send_json_error( array('msg' => __('Enter a valid email.') ) ); }
        elseif( empty($email['phone']) && wp_doing_ajax() ) { wp_send_json_error( array('msg' => __('Enter a valid phone.') ) );}
        else {

            // Validate max length of first and last names
            if(strlen($email['first_name']) > 50 && wp_doing_ajax()) {
                wp_send_json_error( array('msg' => __('Please enter a valid first name.') ));
            }

            if(strlen($email['last_name']) > 50 && wp_doing_ajax()) {
                wp_send_json_error( array('msg' => __('Please enter a valid first name.') ));
            }
...
//				$fieldURL 	= array();
            ...
            // CONNECTION STUFF
            $sfdc_data = array(
                'FirstName' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['first_name'] ) ) ),
                'LastName' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['last_name'] ) ) ),
                'Company' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['company'] ) ) ),
                'Email' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['email'] ) ) ),
                'Country' => ucwords( str_replace('-', ' ', htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['country_code'] ) ) ) ) ),
                'Phone' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['phone'] ) ) ),
                'Note__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['note'] ) ) ),
                'Processor__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['quote-product'] ) ) ),
                'Projected_Quantities__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['quote-quantity'] ) ) ),
                'Product_page__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['Product_page__c'] ) ) ),
                'LeadSource' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['leadsource'] ) ) ),
                'Campaign_medium__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['Campaign_medium__c'] ) ) ),
                'Campaign_source__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['Campaign_source__c'] ) ) ),
                'Campaign_content__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['Campaign_content__c'] ) ) ),
                'Campaign_term__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['Campaign_term__c'] ) ) ),
                'Page_url__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['curl'] ) ) ),
                'Paid_Campaign_Name__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['Paid_Campaign_Name__c'] ) ) ),
                'GA_id__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['GA_id__c'] ) ) )
            );
			...
			$json_encoded = str_replace( array( "\&forall;", "\&part;", "\&exist;", "\&empty;", "\&nabla;", "\&isin;", "\&notin;", "\&ni;", "\&prod;", "\&sum;", "\&Alpha;", "\&Beta;", "\&Gamma;", "\&Delta;", "\&Epsilon;", "\&Zeta;", "\&copy;", "\&reg;", "\&euro;", "\&trade;", "\&larr;", "\&uarr;", "\&rarr;", "\&darr;", "\&spades;", "\&clubs;", "\&hearts;", "\&diams;", "\&lt;br&gt;", "\&lt;", "\&gt;", "&lt;", "&gt;", "&amp;" ), array( "∀", "∂", "∃", "∅", "∇", "∈", "∉", "∋", "∏", "∑", "Α", "Β", "Γ", "Δ", "Ε", "Ζ", "©", "®", "€", "™", "←", "↑", "→", "↓", "♠", "♣", "♥", "♦", " ", "<", ">", "<", ">", "&" ), $json_encoded );
    ...
    if($actionType == 'send_widget_quote') {
        ...
        foreach( $data as $key => $item ){
            if( $key == 'note' ){
                ...
                if(! wp_doing_ajax() && $key === 'System__c') {
                    $email[$key] = sanitize_text_field(implode(';', $item));
                } else {
                    $email[$key] = sanitize_text_field( $item );
                }
...
//		$fieldURL 	= array();
        ...
        $sfdc_data = array(
            ...
            'Company' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['company'] ) ) ),
            'Email' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['email'] ) ) ),
            'Country' => ucwords( str_replace( '-', ' ', htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['country'] ) ) ) ) ),
            'Phone' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['phone'] ) ) ),
            'Note__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['note'] ) ) ),
            'System__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['System__c'] ) ) ),
            'Projected_Quantities__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['Projected_Quantities__c'] ) ) ),
            'Product_page__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['Product_page__c'] ) ) ),
            'LeadSource' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['lead_source'] ) ) ),
            'Campaign_medium__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['Campaign_medium__c'] ) ) ),
            'Campaign_source__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['Campaign_source__c'] ) ) ),
            'Campaign_content__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['Campaign_content__c'] ) ) ),
            'Campaign_term__c' => htmlspecialchars( str_replace( "\n", "\\n", str_replace( '"', "'", $email['Campaign_term__c'] ) ) ),

<?php
class sgLocalBlogSearchMobile extends WP_Widget {
    ...
    function __construct() {
        ...
        parent::__construct('sgLocalBlogSearchMobile', __('Blog Search Mobile Widget (SiteIT)', 'siteitsob'), $widget_ops);
    }
    ...
    function widget($args, $instance) {
        ...
        if( $instance[$prefix.'show_search'] == 'on' ) {
            ...
            <div class="search-section">
                ...
                <form method="get" id="mobile_search_form" action="'.home_url().'">
                    <input type="text" value="" name="s" id="mobile_s" class="form-control" placeholder="'.$instance[$prefix.'search_placeholder'].'" />
                    <input type="hidden" value="<?php echo $term->term_id; ?>" name="cat" />
                    <input type="hidden" value="<?php echo $term->name; ?>" name="catname" />
                    <input type="submit" id="mobile_search_submit" name="Search" class="fa-search" value=""/>
                </form>
            </div>
            ';
        }
...
//add_action( 'widgets_init', create_function('', 'return register_widget("sgLocalBlogSearchMobile");') );
add_action( 'widgets_init', function () {
    return register_widget('sgLocalBlogSearchMobile');
}, 1 );
?>

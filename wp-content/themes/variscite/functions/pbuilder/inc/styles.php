...
class SiteOrigin_Panels_Styles {
	...
	static function get_general_style_fields( $id, $label ) {
		...
		$fields['background_display'] = array(
			...
			'options'     => array(
				...
				'right' 		=> __('Right, with original size', 'siteorigin-panels'),
				'center' 		=> __('Centered, with original size', 'siteorigin-panels'),
				'left' 			=> __('Left, with original size', 'siteorigin-panels'),
		...
		$fields['mobile_background_image_attachment'] = array(
			'name' => __('Mobile Background Image', 'siteorigin-panels'),
			'type' => 'image',
			'group' => 'design',
		...
		$fields['mobile_background_display'] = array(
			'name' => __('Mobile Background Image Display', 'siteorigin-panels'),
			'type' => 'select',
			'group' => 'design',
			'options' => array(
				'tile'	 		=> __('Tiled Image', 'siteorigin-panels'),
	...
	static function row_style_fields( $fields ) {
		...
		$fields['mobile_collapse_behaviour'] = array(
			'name'     => __( 'Mobile Collapse Behaviour', 'siteorigin-panels' ),
			'type'     => 'select',
			'group'    => 'layout',
			'options'  => array(
				''               => __( 'Standard', 'siteorigin-panels' ),
		...
		$fields['hide_in_resolution'] = array(
			'name'     => __( 'Hide In', 'siteorigin-panels' ),
			'type'     => 'select',
			'group'    => 'layout',
			'options'  => array(
				'showInAll'		=> __( 'Show In All Screens', 'siteorigin-panels' ),
				'hideInDesktop'	=> __( 'Hide In Desktop', 'siteorigin-panels' ),
				'hideInTablet'	=> __( 'Hide In Tablet', 'siteorigin-panels' ),
				'hideInMobile'	=> __( 'Hide In Mobile', 'siteorigin-panels' ),
			),
			'priority' => 19,
		);
	...
	static function general_style_attributes( $attributes, $style ){
		...
		if ( ! empty( $style['background_display'] ) && ! empty( $style['background_image_attachment'] ) || !empty($style['mobile_background_image_attachment']) ) {


			/*********************************************
			** CHANGED BY SAGIVE TO INCLUDE MOBILE IMAGE
			** USING MOBBLE PLUGIN AND CUSTOM FIELD 
			** ADDED TO PAGE BUILDER
			*********************************************/


			if( !empty( $style['mobile_background_image_attachment']) ) {
				$url = self::get_attachment_image_src( $style['mobile_background_image_attachment'], 'full' );
				
			}
			else {
				// $url = self::get_attachment_image_src( $style['background_image_attachment'], 'full' );   // CHANGE BY SAGIVE
				$url = !empty($style['background_image_attachment']) ? self::get_attachment_image_src( $style['background_image_attachment'], 'full' ) : '';
			}

			// $url = self::get_attachment_image_src( $style['background_image_attachment'], 'full' );


			if ( ! empty( $url ) && ( $style['background_display'] == 'parallax' || $style['background_display'] == 'parallax-original' ) ) {
		...
		if ( ! empty( $style['background_display'] ) && ! empty( $style['background_image_attachment'] )  || !empty($style['mobile_background_image_attachment']) ) {
			...
			*********************************************/
			...
			if ( ! empty( $url ) ) {
				...
				/*********************************************
				** ADDED BY SAGIVE SEO TO SUPPORT DIFFERENT
				** BACKGROUND IMAGE SETTINGS FOR MOBILE
				*********************************************/
				if( !empty( $style['mobile_background_display']) ) {$customSwithArgs = $style['mobile_background_display']; }
				else {$customSwithArgs = $style['background_display'];}

				switch ( $customSwithArgs ) {
					case 'parallax':

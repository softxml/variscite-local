...
class SiteOrigin_Panels_Renderer {
	...
	private function render_row( $post_id, $ri, & $row, & $panels_data ) {
		...
		$row_attributes = apply_filters( 'siteorigin_panels_row_attributes', array(
			'id'    => 'pg-' . $post_id . '-' . $ri,
			'class' => implode( ' ', $row_classes ).' '.$row['style']['hide_in_resolution'],
		), $row );
		...
		/***************************************
		** SAGIVE
		** if collapse mobile or desktop
		***************************************/
		if( !empty($row[ 'style' ][ 'mobile_collapse_behaviour' ]) ) { $collapseBeh = $row[ 'style' ][ 'mobile_collapse_behaviour' ]; }
		elseif( !empty($row[ 'style' ][ 'collapse_behaviour' ]) ) {$collapseBeh = $row[ 'style' ][ 'collapse_behaviour' ];}
		else {$collapseBeh = '';}

		echo '<div class="innerWrap  cols-'.count($row['cells']).' '.$collapseBeh.'">';		// BY SAGIVE
	...
	private function render_cell( $post_id, $ri, $ci, & $cell, $cells, & $panels_data ) {
		...
		/***STR***************************************************
		// BY SAGIVE - (attaching widget class on row with "-cell")
		********************************************************
		$sgxWidgeXtratCls 	= '';
		$sgxWidgetClsArr 	= wp_list_pluck( $widgets, 'panels_info');
		if(!empty($sgxWidgetClsArr)) {foreach($sgxWidgetClsArr as $sgxClass) {$sgxWidgeXtratCls .= $sgxClass['class'].'-cell';} }
		$cell_classes[] 		= $sgxWidgeXtratCls;
		*/
		/***END***************************************************/
